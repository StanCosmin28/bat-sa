  ...
  <AnimatedSection delay={delay}>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-bat-navy font-semibold text-sm sm:text-base truncate">{value}</p>
      </div>
      {href && (
        <ArrowUpRight size={18} className="text-gray-300 group-hover:text-bat-blue group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-300 shrink-0" />
      )}
    </a>
  </AnimatedSection>
);
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <motion.div
                  ...
                  className="bg-white border border-gray-100 rounded-2xl p-4 text-center shadow-[0_4px_20px_-8px_rgba(0,0,0,0.05)]"
                >
                  <p className="text-xl sm:text-2xl font-black text-bat-navy">{item.value}</p>
                  <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-gray-400 mt-1">{item.label}</p>
                </motion.div>
              ))}
            </div>
          </AnimatedSection>
        ...
        <AnimatedSection delay={0.4} className="mt-20 lg:mt-24">
          <div className="relative overflow-hidden bg-bat-navy rounded-3xl px-8 py-12 lg:px-16 lg:py-14 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            ...
            <div className="relative z-10">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-bat-blue/80 mb-2">Direct contact</p>
              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black text-white tracking-tight">
                Prefer to reach out directly?
              </h2>
              <p className="text-gray-400 mt-2 text-sm sm:text-base font-light">Our team typically responds within one business day.</p>
            </div>
